function out = fnA_Delay(gamma_vec,d1_vec,theta_vec,r_vec,alpha,x_t,lambda,p_t)
    gamma_all = [];
    for gamma = gamma_vec
        d1_all1 = [];
        d1_all2 = [];
        for d1 = d1_vec
            for i = 1:length(theta_vec)
                ...
                first_I1(valid_C) = exp(2*lambda*p_t*gamma*((d1^alpha)./(sqrt(gamma*(d1^alpha)*(1-p_t) + ...
                            (x_t*cos(theta) - r_vec1).^2))).*((pi/2) - atan(sqrt(((d1^alpha) - ...
                            (x_t*cos(theta) - r_vec1).^2)./(gamma*(d1^alpha)*(1-p_t) + (x_t*cos(theta) - r_vec1).^2)))));

                first_I1(Nvalid_C) = 0;

                first_I2(Nvalid_C) = exp(2*lambda*pi*p_t*gamma*(d1^alpha)./(2*sqrt(gamma*(d1^alpha)*(1-p_t) + (x_t*cos(theta) - r_vec2).^2)));
                first_I2(valid_C) = 0;
